...
// 指定上级菜单添加
async function handleAddById(row: Menu) {
...
// 拷贝当前菜单
async function handleCopy(row: Menu) {
  state.menuForm.data = { ...row }
...
// 状态变更
async function handleStatusChange(row: Menu) {
  const data = {
    id: row.id,
    status: row.status,
  }
...
// 全部展开/全部折叠
function toggleRowExpansionAll(dataList: Menu[] | undefined, value: boolean) {
  if (!dataList) {
    return
  }
  dataList.forEach((v) => {
    ...
    >
      ...
      >
        <template #default="scope">
          ...
          <el-tag v-else-if="scope.row.hidden">
            隐藏
          </el-tag>
      ...
      >
        <template #default="scope">
          <el-tag v-if="scope.row.always_show" type="success">
            显示
          </el-tag>
          <el-tag v-else>
            隐藏
