...
// 菜单列表转为路由列表
export function asyncRoutesByMenus(menus: Menu[]) {
...
// 过滤异步路由
export function filterAsyncRouter(menus: Menu[]) {
...
// 按钮权限
export function buttonPermissions(permissions: ButtonPermission[]) {
...
// 是否存在按钮权限
export function hasButtonPermission(value: string): boolean {
...
// 按钮是否禁用
export function isDisabledButton(value: string): boolean {
...
export function resetRouter() {
  // 移除之前存在的路由
  const routeNameSet: Set<string | symbol> = new Set()
